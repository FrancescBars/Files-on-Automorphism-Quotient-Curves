...
Then we rewrite a program to compute the points of the quotient curve $X_0(N)/W$ over a finite field $\mathbb{F}_{q^k}$ (q is a prime not dividing N). Then we write a program to check the sign of $|X_0(N)/W(F_{q^k})|-degree*|P^1(F_{q^k})|$, where ``degree`` denotes a natural number. 
The command ``MapdegreedtoP1(q,degree,bound,FpnpointsQuotientCurve)`` list if any of the set $(q^k, z)$ with $k<\mathrm{bound}$ such that $|X_0(N)/W(F_{q^k})|-degree*|P^1(F_{q^k})|=z$, where z is a positive integer. Thus no degree map between the quotient modular curve $X_0(N)/W$ to the projective space $\mathbb{P}^1$ defined over $\mathbb{F}_{q^k}$, if the list is non empty (recall q does not divide N).  
