...
Then we rewrite a program to compute the points of the quotient curve $X_0(N)/W$ over a finite field $\mathbb{F}_{q^k}$ (q is a prime not dividing N). Then we write a program to check the sign of $|X_0(N)/W(\mathbb{F}_{q^k})|-degree*|P^1(\mathbb{F}_{q^k})|$, where ``degree`` denotes a natural number. 
