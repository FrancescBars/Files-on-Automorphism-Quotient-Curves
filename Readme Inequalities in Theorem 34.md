...
Then we rewrite a program to compute the points of the quotient curve $X_0(N)/W$ over a finite field $F_{q^k}$ (q is a prime not dividing N, and $F_{q^k}$ denotes the field with $q^k$ elements). 
Then we write a program to check the sign of $|X_0(N)/W (F_{q^k})|-degree \cdot |\mathbb{P}^1 (F_{q^k})|$, where ``degree`` denotes a natural number. 
The command ``MapdegreedtoP1(q,degree,bound,FpnpointsQuotientCurve)`` list if any of the set $(q^k, z)$ with $k<\mathrm{bound}$ such that $|X_0(N)/W (F_{q^k})|-degree \cdot |\mathbb{P}^1 (F_{q^k})|=z$, where z is a positive integer. Thus no $degree$ map between the quotient modular curve $X_0(N)/W$ to the projective space $\mathbb{P}^1$ defined over $F_{q^k}$, if the list is non empty (recall q does not divide N).  
...
$|X_0(N)/W (F_{q^k})|-degree \cdot |\mathbb{P}^1 (F_{q^k})|=z$,
where z is a positive integer ($F_{q^k}$ denotes the field with $q^k$ elements). Thus no $degree$ map between the quotient modular curve $X_0(N)/W$ to the projective space $\mathbb{P}^1$ defined over $(F_{q^k})$, if the list is non empty (recall q is a prime not dividing N).
````magma
